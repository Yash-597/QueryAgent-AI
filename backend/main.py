...
async def get_schema():
    """Returns tables with columns for the interactive Schema Explorer."""
    try:
        tables = sorted(schema_provider.get_table_names())
        result = [
            {"name": table, "columns": schema_provider.get_columns(table)}
            for table in tables
        ]
